...
export default function TrackingPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-indigo-500/30 pt-8 pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <div>
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight flex items-center gap-4 mb-2">
              Tracking Details
              <AWBStatusBadge status={mockData.status} />
            </h1>
            <div className="flex items-center gap-3 text-slate-600">
              <span className="flex items-center gap-1.5 bg-slate-100/50 border border-blue-200 px-2.5 py-1 rounded-md text-sm">
                 <Hash className="w-3.5 h-3.5 text-indigo-400" />
                 AWB: <span className="font-mono font-medium text-slate-900">{mockData.awb}</span>
              </span>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="xl:col-span-2 space-y-8">
            ...
            <motion.div
              ...
              className="bg-white backdrop-blur-xl border border-blue-200/80 rounded-3xl p-6 lg:p-8 relative overflow-hidden shadow-2xl"
            >
              ...
              <div className="flex items-center justify-between mb-10">
                 ...
                 <div className="text-sm text-slate-600 bg-slate-100/50 px-3 py-1.5 rounded-lg border border-blue-200 flex items-center gap-2">
              ...
              <div className="relative pl-2">
                ...
                <div className="absolute left-[23px] top-4 bottom-4 w-0.5 bg-slate-100/80 rounded-full"></div>
                {/* Progress Line */}
                <div
                  ...
                  {STAGES.map((stage, index) => {
                    ...
                    return (
                      <div key={stage} className="relative flex items-center gap-8 group">
                        {/* Icon/Circle */}
                        <div className="relative z-10 bg-white rounded-full py-2">
                          {isCompleted && (
                            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="bg-slate-100/50 rounded-full p-1 border border-blue-200">
                              <CheckCircle2 className={`w-6 h-6 ${isDelivered || mockData.status === 'Failed' ? 'text-slate-600' : 'text-indigo-400'}`} />
                            </motion.div>
                          )}
                          {isCurrent && (
                          ...
                          {isPending && (
                            <div className="w-8 h-8 flex items-center justify-center ml-0.5">
                               <div className="w-2.5 h-2.5 rounded-full bg-slate-700 border border-blue-200"></div>
                            </div>
                          )}
                        </div>
                        ...
                        <div className={`flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-2 ${isPending ? 'opacity-40' : ''}`}>
                          <h3 className={`text-base font-medium ${isCurrent ? (isDelivered ? 'text-emerald-400' : mockData.status === 'Failed' ? 'text-rose-400' : 'text-indigo-400') : isCompleted ? 'text-slate-900' : 'text-slate-500'}`}>
                            {stage}
                          </h3>
                          {(isCompleted || isCurrent) && (
                            <div className="text-sm font-mono text-slate-500 bg-slate-100/30 px-2 py-0.5 rounded border border-blue-200/30 w-fit">
            ...
            <motion.div
              ...
              className="bg-white backdrop-blur-xl border border-blue-200/80 rounded-3xl p-6 lg:p-8 shadow-xl"
            >
              <div className="flex items-center justify-between mb-6">
                 ...
                 <span className="text-xs font-medium px-2.5 py-1 bg-slate-100/80 text-slate-700 rounded-lg border border-blue-200">{scanEvents.length} Events</span>
              </div>
              ...
              <div className="space-y-3">
                  ...
                  {visibleEvents.map((event, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl border transition-all ${isException ? 'bg-rose-500/5 border-rose-500/20 shadow-[0_4px_20px_rgba(244,63,94,0.05)]' : 'bg-slate-50 border-slate-200 hover:bg-slate-100/40 hover:border-slate-300'}`}
                      >
                        <div className="flex flex-col sm:flex-row gap-3 sm:gap-6 sm:items-center">
                          <div suppressHydrationWarning className={`text-sm font-mono w-40 bg-black/20 px-3 py-1.5 rounded-lg border border-white/5 ${isException ? 'text-rose-400' : 'text-slate-600'}`}>
                          ...
                          <div>
                            <div className={`font-medium ${isException ? 'text-rose-400 font-bold' : 'text-slate-900'}`}>{event.status}</div>
                        ...
                        <div className="mt-3 sm:mt-0 text-xs px-2.5 py-1 bg-black/40 rounded-md text-slate-600 border border-blue-200 max-w-max flex items-center gap-1.5">
                ...
                {scanEvents.length > 3 && (
                  <button
                    ...
                    className="w-full mt-4 py-3 text-sm font-medium text-slate-700 hover:text-slate-900 flex items-center justify-center gap-2 rounded-2xl hover:bg-slate-100/50 transition-colors border border-dashed border-blue-200/80 bg-slate-50"
                  >
                    {showAllEvents ? (
                      <>
          ...
          <div className="space-y-6">
            <motion.div
              ...
              className="bg-white backdrop-blur-xl border border-blue-200/80 rounded-3xl overflow-hidden sticky top-8 shadow-2xl"
            >
              {/* Routing Visual Card */}
              <div className="relative p-8 bg-gradient-to-b from-slate-800/40 to-transparent border-b border-blue-200 overflow-hidden">
                ...
                <div className="flex items-center justify-between relative z-10">
                  <div className="text-center w-24">
                    <div className="text-4xl font-black text-slate-900 tracking-tighter">{mockData.origin}</div>
                    <div className="text-xs text-slate-600 mt-1.5 truncate font-medium">{mockData.originCity}</div>
                  </div>
                  <div className="flex-1 flex items-center justify-center relative px-4">
                    ...
                    <motion.div 
                       ...
                       className={`absolute bg-white px-2 p-1 rounded-full border border-slate-200 ${isDelivered ? 'text-emerald-500' : 'text-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.3)]'}`}
                    >
                       <Plane className="w-5 h-5" />
                    </motion.div>
                  </div>
                  <div className="text-center w-24">
                    <div className="text-4xl font-black text-slate-900 tracking-tighter">{mockData.destination}</div>
                    <div className="text-xs text-slate-600 mt-1.5 truncate font-medium">{mockData.destinationCity}</div>
                  </div>
                </div>
              </div>

              {/* Core Logistics Info */}
              <div className="p-6 space-y-4 border-b border-blue-200 bg-slate-50">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-slate-100/30 p-3.5 rounded-2xl border border-blue-200 hover:bg-slate-100/50 transition-colors">
                    ...
                    <div className="text-sm font-bold text-slate-900">{mockData.carrier}</div>
                  </div>
                  <div className="bg-slate-100/30 p-3.5 rounded-2xl border border-blue-200 hover:bg-slate-100/50 transition-colors">
                    <div className="text-xs text-slate-500 flex items-center gap-1.5 mb-1.5 font-medium">
                      <FileText className="w-3.5 h-3.5 text-amber-400" /> Flight No.
                    </div>
                    <div className="text-sm font-bold text-slate-900">{mockData.flightNo}</div>
                  </div>
                  <div className="bg-slate-100/30 p-3.5 rounded-2xl border border-blue-200 hover:bg-slate-100/50 transition-colors">
                    <div className="text-xs text-slate-500 flex items-center gap-1.5 mb-1.5 font-medium">
                      <Package className="w-3.5 h-3.5 text-emerald-400" /> Pieces
                    </div>
                    <div className="text-sm font-bold text-slate-900">{mockData.pieceCount} NOP</div>
                  </div>
                  <div className="bg-slate-100/30 p-3.5 rounded-2xl border border-blue-200 hover:bg-slate-100/50 transition-colors">
                    ...
                    <div className="text-sm font-bold text-slate-900">{mockData.weight} <span className="text-xs font-normal text-slate-500">kg</span></div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 space-y-5 bg-slate-100">
                <div className="flex items-start gap-3.5">
                   ...
                   <div className="flex-1">
                      ...
                      <div suppressHydrationWarning className={`text-sm font-bold ${isDelivered ? 'text-emerald-400' : 'text-slate-900'}`}>{mockData.estimatedDelivery}</div>
                   </div>
                </div>
                
                <div className="flex items-start gap-3.5">
                   <div className="w-10 h-10 rounded-xl bg-slate-100/50 flex items-center justify-center flex-shrink-0 border border-blue-200">
                      <User className="w-5 h-5 text-slate-600" />
                   </div>
                   <div className="flex-1">
                      <div className="text-xs text-slate-500 mb-1 font-medium">Consignee</div>
                      <div className="text-sm font-bold text-slate-900">{mockData.consignee}</div>
                   </div>
                </div>
                ...
                <div className="flex items-start gap-3.5">
                   ...
                   <div className="flex-1 flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-sm font-bold text-slate-900">{mockData.serviceLevel}</div>
                      </div>
                      <span className="text-[10px] uppercase font-bold px-2 py-1 bg-slate-100 rounded text-slate-600 border border-blue-200">{mockData.incoterm}</span>
                   </div>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="bg-white backdrop-blur-xl border border-blue-200/80 rounded-3xl p-6 shadow-xl space-y-4"
            >
               <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wider mb-2">Compliance & Sustainability</h3>
               
               <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-blue-200">
                  <div className="flex items-center gap-3">
                     <FileCheck className="w-4 h-4 text-slate-600" />
                     <span className="text-sm text-slate-700">Customs Status</span>
                  </div>
               ...
               <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-blue-200">
                  <div className="flex items-center gap-3">
                     ...
                     <span className="text-sm text-slate-700">Cargo Insurance</span>
                  </div>
               ...
               <div className="flex items-center justify-between p-3 bg-emerald-900/10 rounded-xl border border-emerald-900/30">
                  <div className="flex items-center gap-3">
                     ...
                     <span className="text-sm text-slate-700">Est. CO₂ Footprint</span>
                  </div>
