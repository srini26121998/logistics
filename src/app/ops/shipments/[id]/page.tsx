    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">LR Number: {shipment.lrNumber} • Booked: {shipment.bookedDate}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white/80 backdrop-blur-md border border-blue-200 rounded-2xl p-6">
            <h2 className="text-lg font-semibold text-slate-900 mb-6">Route Information</h2>
            
            <div className="flex items-center justify-between">
              <div className="text-center w-24">
                <div className="text-3xl font-black text-slate-900">{shipment.origin}</div>
              </div>
              <div className="flex-1 flex items-center justify-center relative px-2">
                <div className="w-full border-t-2 border-dashed border-blue-200"></div>
                <div className="absolute bg-white px-2 text-indigo-500">
                  {shipment.mode === 'Air' ? <Plane className="w-6 h-6" /> : <Truck className="w-6 h-6" />}
                </div>
              </div>
              <div className="text-center w-24">
                <div className="text-3xl font-black text-slate-900">{shipment.destination}</div>
              </div>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8 pt-6 border-t border-blue-200">
              <div>
                <div className="text-xs text-slate-500 mb-1">Carrier</div>
                <div className="text-sm font-semibold text-slate-900">{shipment.carrier}</div>
              </div>
              <div>
                <div className="text-xs text-slate-500 mb-1">Flight/Vehicle</div>
                <div className="text-sm font-semibold text-slate-900">{shipment.flight || 'TBA'}</div>
              </div>
              <div>
                <div className="text-xs text-slate-500 mb-1">Total Pieces</div>
                <div className="text-sm font-semibold text-slate-900">{shipment.pieces} NOP</div>
              </div>
              <div>
                <div className="text-xs text-slate-500 mb-1">Gross Weight</div>
                <div className="text-sm font-semibold text-slate-900">{shipment.weight}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-md border border-blue-200 rounded-2xl p-6">
            <h2 className="text-lg font-semibold text-slate-900 mb-6">Manual Status Update</h2>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="bg-white border border-blue-200 rounded-lg p-3">
                  <div className="font-semibold text-slate-900">{shipment.shipper}</div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-white border border-blue-200 rounded-lg p-3">
                  <div className="font-semibold text-slate-900">{shipment.consignee}</div>
                </div>
              </div>
            </div>
          </div>

          <Link href={`/track/${shipment.awb}`} target="_blank" className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-900 rounded-xl text-sm font-medium transition-colors border border-blue-200">
