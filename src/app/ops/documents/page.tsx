  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600 mt-1">Upload and manage commercial invoices, packing lists, and KYC documents.</p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <form 
              ...
              className={`relative border-2 border-dashed rounded-2xl p-12 text-center transition-all flex flex-col items-center justify-center min-h-[300px] ${
                dragActive 
                  ...
                  : "border-slate-300 bg-slate-50/50 hover:border-slate-300 hover:bg-white"
              }`}
            >
              <input 
                ref={inputRef} 
              ...
              <h3 className="text-xl font-bold text-slate-900 mb-2">Drag & Drop files here</h3>
              <p className="text-sm text-slate-600 mb-6">Support for PDF, JPG, PNG, DOC, XLS up to 50MB</p>
              
              <button 
                type="button" 
                onClick={() => inputRef.current?.click()}
                className="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-900 rounded-xl font-medium transition-colors border border-blue-200"
              >
                Browse Files
              </button>
            </form>
          ...
          {uploads.length > 0 && (
            <motion.div 
              ...
              className="bg-white/80 backdrop-blur-md border border-blue-200 rounded-2xl p-6"
            >
              <h2 className="text-lg font-semibold text-slate-900 mb-4">Upload Queue</h2>
                ...
                <AnimatePresence>
                  {uploads.map(upload => (
                    <motion.div 
                      ...
                      className="bg-white border border-blue-200 rounded-xl p-4 overflow-hidden relative"
                    >
                      ...
                      <div className="flex items-center gap-4 relative z-10">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-slate-900 truncate text-sm">{upload.name}</div>
          ...
          >
            ...
            <div className="flex-1 overflow-auto p-4 pt-0 custom-scrollbar space-y-2">
              ...
              {[
                ...
                <div key={i} className="group p-3 hover:bg-white rounded-xl border border-transparent hover:border-blue-200 transition-colors cursor-pointer flex items-center gap-3">
                  <div className="w-8 h-8 rounded bg-slate-100 flex items-center justify-center text-slate-600 group-hover:text-indigo-400 group-hover:bg-indigo-500/10 transition-colors shrink-0">
                    <File className="w-4 h-4" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-medium text-slate-900 truncate group-hover:text-slate-900 transition-colors">{file.name}</div>
